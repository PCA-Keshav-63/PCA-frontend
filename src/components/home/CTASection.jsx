...
function CTASection() {
  return (
    // Changed section background to white and removed previous gradient classes
    <section className="py-24 bg-white relative overflow-hidden">
      {/* Background Elements (circular blurs) removed to achieve a clean white background */}

      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        {/* Badge: Adjusted background and text color for visibility on a white section background */}
        <div className="inline-flex items-center space-x-2 bg-indigo-50 text-indigo-700 rounded-full px-6 py-3 mb-8">
          <Sparkles className="w-5 h-5 text-yellow-500" /> {/* Adjusted Sparkles color for better contrast */}
          <span className="font-medium">Join Thousands of Happy Users</span>
        </div>

        {/* Heading: Changed text color from white to dark indigo for readability */}
        <h2 className="text-4xl md:text-6xl font-bold text-indigo-900 mb-6">
          Ready to Get
          <span className="block bg-gradient-to-r from-yellow-300 to-pink-300 bg-clip-text text-transparent">
            Started?
          </span>
        ...
        {/* Paragraph: Changed text color from a light indigo to a darker gray for readability */}
        <p className="text-xl text-gray-700 mb-12 max-w-3xl mx-auto leading-relaxed">
          Join our community of customers and businesses. Start discovering amazing local services today.
        </p>

        <div className="flex flex-col sm:flex-row gap-6 justify-center max-w-md mx-auto">
          {/* Find Services Button: Changed background to indigo and text to white so it stands out on a white section background */}
          <Link
            to="/login"
            className="group bg-indigo-600 text-white px-8 py-4 rounded-xl font-semibold hover:shadow-xl transform hover:scale-105 transition-all duration-200 flex items-center justify-center space-x-2"
          >
            <span>Find Services</span>
            <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
          </Link>
          {/* List Your Business Button: Kept as is, as its gradient background works well on a white section background */}
          <Link
            to="/login"
            className="group bg-gradient-to-r from-yellow-400 to-orange-500 text-white px-8 py-4 rounded-xl font-semibold hover:shadow-xl transform hover:scale-105 transition-all duration-200 flex items-center justify-center space-x-2"
          >
        ...
        {/* Trust Indicators: Changed text color from light indigo to a darker gray for readability */}
        <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto">
          {[
            "Trusted by 50K+ Businesses",
            "200K+ Happy Customers",
